...
> **🚀 要创建项目？** → 请先阅读 [PROJECT_SCAFFOLDING_GUIDE.md](../PROJECT_SCAFFOLDING_GUIDE.md)
> 
> ⚠️ **重要提醒**: YellowBull Agent 是一个 AI Agent 框架（对话 → 意图识别 → 工具调用 → 回复），**不是** CRUD 应用，MVP 阶段不需要数据库。
...
| 5 | [Tool Executor](./05-tool-executor.md) | `core/tool_executor.py` | 超时控制、指数退避重试、并行/串行调度、结果缓存、错误隔离 |
...
```
     ...
     └────┬────┘       └────┬────┘       └────┬────┘
          ...
          └────────┬─────────┼────────────────┘
                   ▼         ▼
            ┌─────────────┐ ┌──────────────┐
            │  Agent Core │ │ Tool Executor│ ← 超时/重试/并行调度
            └──────┬──────┘ └──────┬───────┘
                   │               │
                   └───────┬───────┘
                           ▼
